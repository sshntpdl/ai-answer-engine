      ...
      async start(controller) {
        ...
        try {
          ...
          const fuResponse = await llm.invoke([
            new SystemMessage(
              "Generate exactly 4 concise follow-up questions as a JSON array. Return ONLY the JSON array.",
            ),
            new HumanMessage(
          ...
          try {
            const text = (fuResponse.content as string)
              .replace(/```json\n?|\n?```/g, "")
              .trim();
          ...
          const saved = await searchRepository.create(
            query,
            fullAnswer,
            sources,
          );
